...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">Edit Gift Card</h4>
...
<form method="POST" action="{{ route('admin.marketing.gift-cards.update', $giftCard->id) }}" id="itemForm">
    @csrf
    @method('PUT')
    ...
    <div class="row">
            ...
            <div class="card border-0 shadow-sm mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="title" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $giftCard->title) }}" required>
                        @error('title')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="description" name="description" class="form-control @error('description') is-invalid @enderror" rows="3">{{ old('description', $giftCard->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @else
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="code" class="form-control" value="{{ $giftCard->code }}" readonly>
                ...
                <div class="card-body">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" id="balance" name="balance" class="form-control @error('balance') is-invalid @enderror" value="{{ old('balance', $giftCard->balance) }}" min="0" step="0.01" required>
                            </div>
                            @error('balance')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @else
                                <div class="form-text">Current balance: {{ formatPrice($giftCard->balance) }} (Initial: {{ formatPrice($giftCard->initial_amount) }})</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Usage Count</label>
                            <div class="form-control-plaintext">
                                <span class="badge bg-secondary">{{ $giftCard->usage_count }}</span>
                                @if($giftCard->usage_limit)
                                / {{ $giftCard->usage_limit }}
                                @else
                                / ∞
                                @endif
                            </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <select id="discount_type" name="discount_type" class="form-select @error('discount_type') is-invalid @enderror" required>
                                <option value="percent" {{ old('discount_type', $giftCard->discount_type) === 'percent' ? 'selected' : '' }}>Percentage (%)</option>
                                <option value="fixed" {{ old('discount_type', $giftCard->discount_type) === 'fixed' ? 'selected' : '' }}>Fixed Amount</option>
                            </select>
                            @error('discount_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @endif
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                <span class="input-group-text">{{ config('app.currency_symbol', '$') }}</span>
                                <input type="number" id="min_order_amount" name="min_order_amount" class="form-control @error('min_order_amount') is-invalid @enderror" value="{{ old('min_order_amount', $giftCard->min_order_amount) }}" min="0" step="0.01">
                            </div>
                    ...
                    <div class="row g-3 mt-2">
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <span class="input-group-text">{{ config('app.currency_symbol', '$') }}</span>
                                <input type="number" id="max_discount_amount" name="max_discount_amount" class="form-control @error('max_discount_amount') is-invalid @enderror" value="{{ old('max_discount_amount', $giftCard->max_discount_amount) }}" min="0" step="0.01">
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" id="usage_limit" name="usage_limit" class="form-control @error('usage_limit') is-invalid @enderror" value="{{ old('usage_limit', $giftCard->usage_limit) }}" min="1">
                            @error('usage_limit')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @else
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="user_id" name="user_id" class="form-select @error('user_id') is-invalid @enderror">
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('user_id', $giftCard->user_id) == $user->id ? 'selected' : '' }}>
        ...
        <div class="col-lg-4">
            ...
            <div class="card border-0 shadow-sm mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" id="expiry_date" name="expiry_date" class="form-control @error('expiry_date') is-invalid @enderror" value="{{ old('expiry_date', $giftCard->expiry_date?->format('Y-m-d')) }}">
                        @error('expiry_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @else
                    ...
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $giftCard->is_featured) ? 'checked' : '' }}>
                    ...
                    <div class="mb-0">
                        ...
                        <div class="input-group">
                            <input type="color" id="background_color" name="background_color" class="form-control form-control-color" value="{{ old('background_color', $giftCard->background_color ?? '#6366f1') }}">
                            <input type="text" id="background_color_text" class="form-control" value="{{ old('background_color', $giftCard->background_color ?? '#6366f1') }}">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mb-3">
                ...
                <div class="card-body">
                    <textarea id="terms_conditions" name="terms_conditions" class="form-control @error('terms_conditions') is-invalid @enderror" rows="4" placeholder="Enter terms and conditions">{{ old('terms_conditions', $giftCard->terms_conditions) }}</textarea>
            ...
            <div class="card border-0 shadow-sm mb-3">
                ...
                <div class="card-body">
                    <ul class="list-unstyled mb-0 small">
                        <li class="mb-2"><strong>Created:</strong> {{ $giftCard->created_at->format('d M, Y h:i A') }}</li>
                        <li class="mb-2"><strong>Updated:</strong> {{ $giftCard->updated_at->format('d M, Y h:i A') }}</li>
                        <li><strong>Initial Amount:</strong> {{ formatPrice($giftCard->initial_amount) }}</li>
                    </ul>
                </div>
            </div>
...
<div class="floating-save-container">
    ...
    <button type="submit" form="itemForm" class="btn btn-primary floating-save-btn">
        <i class="bi bi-check-lg me-1"></i> Update Gift Card
    </button>
</div>
@endsection
