...
@section('title', 'Manage Products - ' . $flashDeal->title)
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <small class="text-muted">{{ $flashDeal->title }}</small>
    </div>
...
<form method="POST" action="{{ route('admin.marketing.flash-deals.products.update', $flashDeal->id) }}" id="itemForm">
    @csrf
                            ...
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    @foreach($products as $product)
                                        @php
                                            $flashDealProduct = $flashDeal->products->firstWhere('product_id', $product->id);
                                            $isSelected = $flashDealProduct !== null;
                                            $discount = $flashDealProduct ? $flashDealProduct->discount : '';
                                            $discountType = $flashDealProduct ? $flashDealProduct->discount_type : 'percent';
                                        @endphp
                                        <tr class="{{ $isSelected ? 'table-primary' : '' }}">
                                            ...
                                            <td>
                                                <span class="text-primary">{{ formatPrice($product->unit_price) }}</span>
                                                @if($product->discount > 0)
                                                    <br><small class="text-success">-{{ formatPrice($product->discount) }}</small>
                                                @endif
                                            </td>
            ...
            <div class="card border-0 shadow-sm mb-3">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <span class="badge bg-{{ $flashDeal->status === 'active' ? 'success' : 'secondary' }}">
                            {{ ucfirst($flashDeal->status) }}
                        </span>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
                        <span class="text-muted">Start:</span>
                        <strong>{{ $flashDeal->start_date->format('M d, Y h:i A') }}</strong>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
                        <span class="text-muted">End:</span>
                        <strong>{{ $flashDeal->end_date->format('M d, Y h:i A') }}</strong>
                    </div>
                    <hr>
                    <div class="d-flex justify-content-between mb-0">
                        <span class="text-muted">Selected Products:</span>
                        <strong id="selectedCount">{{ $flashDeal->products->count() }}</strong>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mb-3">
                ...
                <div class="card-body">
                    <ol class="mb-0 small text-muted">
                        <li class="mb-2">Check the products you want to add to this flash deal</li>
                        <li class="mb-2">Enter discount value (percentage or fixed amount)</li>
                        <li class="mb-2">Click "Update Products" to save changes</li>
                        <li class="mb-0">Discounted price will be calculated automatically</li>
                    </ol>
                </div>
            </div>
