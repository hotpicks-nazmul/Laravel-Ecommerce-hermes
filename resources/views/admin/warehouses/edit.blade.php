...
@push('scripts')
<script>
    // Existing city IDs for pre-selection in edit mode
    const existingCityIds = @json($warehouse->cities->pluck('id'));
                ...
                if (data.success && data.cities && data.cities.length > 0) {
                    ...
                    Object.keys(grouped).sort().forEach(stateName => {
                        ...
                        grouped[stateName].forEach(c => {
                            ...
                            const isOwnCity = existingCityIds.includes(c.id);

                            if (c.assigned_to && !isOwnCity) {
