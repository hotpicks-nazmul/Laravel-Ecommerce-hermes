...
<script>
    ...
    // Handle pagination clicks via event delegation (after AJAX refresh)
    document.addEventListener('click', function(e) {
        const paginationLink = e.target.closest('.card-footer .pagination a');
        if (paginationLink) {
            e.preventDefault();
            const url = new URL(paginationLink.href);
            const params = new URLSearchParams(url.search);
            // Update filter form params from the pagination URL
            filterForm.querySelector('[name="search"]').value = params.get('search') || '';
            filterForm.querySelector('[name="status"]').value = params.get('status') || '';
            filterForm.querySelector('[name="warehouse_id"]').value = params.get('warehouse_id') || '';
            filterForm.querySelector('[name="per_page"]').value = params.get('per_page') || '10';
            fetchResults();
        }
    });
</script>
